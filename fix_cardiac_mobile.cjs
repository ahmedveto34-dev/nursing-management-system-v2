const fs = require('fs');
let code = fs.readFileSync('src/components/CardiacView.tsx', 'utf8');
...
const targetTableRows = `                    <td className="p-4 text-sm">{item.date}</td>
...
const replacementRows = `                    <td className="p-4 text-sm">{item.date}</td>
                    <td className="p-4 font-medium">{item.patientId}</td>
...
fs.writeFileSync('src/components/CardiacView.tsx', code);
