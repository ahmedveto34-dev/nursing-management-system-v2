  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isDuplicate) {
        ...
        addInfection(payload).catch(err => {
          console.error(err);
        });
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 print:hidden">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input required name="infectionType" type="text" placeholder="مثال: VAP, CLABSI, CAUTI" className="w-full rounded-lg border-gray-300 border p-2 focus:ring-2 focus:ring-emerald-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">مكان العدوى (Site)</label>
            <input required name="infectionSite" type="text" className="w-full rounded-lg border-gray-300 border p-2 focus:ring-2 focus:ring-emerald-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">بروتوكول العزل</label>
            <select required name="isolationProtocol" className="w-full rounded-lg border-gray-300 border p-2 focus:ring-2 focus:ring-emerald-500">
