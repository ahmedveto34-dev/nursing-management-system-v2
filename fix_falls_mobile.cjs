const fs = require('fs');
let code = fs.readFileSync('src/components/FallsView.tsx', 'utf8');
              ...
              <div key={i} className="p-4 space-y-3 hover:bg-gray-50">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm bg-gray-50 p-3 rounded-lg">
                  ...
                  <div className="col-span-2">
                    <span className="text-gray-500 block text-xs">الإجراء</span>
                    <span className="font-medium">{item.postFallAction}</span>
                  </div>
                </div>
              </div>
            ))
...
fs.writeFileSync('src/components/FallsView.tsx', code);
